...
namespace ABPvNextOrangeAdmin.Config
{
    ...
    { 
        public IConfigurationProvider Build(IConfigurationBuilder builder)=> new CaptchaProvider();
    }
    ...
    public class CaptchaProvider : ConfigurationProvider
    {
        ...
        {
            if (ProjectConfig.CaptchaType == "math")
            {
                Data = getCaptchaPropertiesMath();
            }
            else if (ProjectConfig.CaptchaType == "char")
            {
